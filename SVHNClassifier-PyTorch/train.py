...
parser.add_argument('-p', '--patience', default=-1, type=int, help='Default 100, set -1 to train infinitely')
...
def _loss(length_logits, digits_logits, length_labels, digits_labels):
    ...
    digit4_cross_entropy = torch.nn.functional.cross_entropy(digits_logits[3], digits_labels[3])
    digit5_cross_entropy = torch.nn.functional.cross_entropy(digits_logits[4], digits_labels[4])
    digit6_cross_entropy = torch.nn.functional.cross_entropy(digits_logits[5], digits_labels[5])
    digit7_cross_entropy = torch.nn.functional.cross_entropy(digits_logits[6], digits_labels[6])
    digit8_cross_entropy = torch.nn.functional.cross_entropy(digits_logits[7], digits_labels[7])
    digit9_cross_entropy = torch.nn.functional.cross_entropy(digits_logits[8], digits_labels[8])
    digit10_cross_entropy = torch.nn.functional.cross_entropy(digits_logits[9], digits_labels[9])
    loss = length_cross_entropy + digit1_cross_entropy + digit2_cross_entropy + digit3_cross_entropy + digit4_cross_entropy + digit5_cross_entropy \
           + digit6_cross_entropy + digit7_cross_entropy + digit8_cross_entropy + digit9_cross_entropy + digit10_cross_entropy
        ...
        for batch_idx, (images, length_labels, digits_labels) in enumerate(train_loader):
            ...
            if step % num_steps_to_show_loss == 0:
                ...
                print('=> %s: step %d, loss = %f, learning_rate = %f (%.1f examples/sec)' % (
                    datetime.now(), step, loss.data.item(), learning_rate, examples_per_sec))
